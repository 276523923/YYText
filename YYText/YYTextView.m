    ...
    [_dataDetector enumerateMatchesInString:text.string options:kNilOptions range:NSMakeRange(0, text.length) usingBlock: ^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                detected = YES;
                if (self.highlightTextAttributes.count) {
                    YYTextHighlight *highlight = [YYTextHighlight highlightWithAttributes:self.highlightTextAttributes];
                    [text yy_setTextHighlight:highlight range:result.range];
                }
                if (self.linkTextAttributes.count) {
                    [self.linkTextAttributes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        ...
        NSInteger offset = _selectedTextRange.end.offset;
        if (offset > _innerText.length) {
            offset = _innerText.length;
        }
        [_innerText replaceCharactersInRange:NSMakeRange(offset, 0) withString:markedText];
